...
public:
    ...
    //rescuively copying each node from top to bottom.
    void copyNode(Node*& sub, Node* target){
        if(target != nullptr){
            sub = new Node(*target);
            copyNode(sub->left_, target->left_);
            copyNode(sub->right_, target->right_);
        }
    }
	void remove(const T& data){
        root_ = removeInR(root_, data);
	}
//---
    
    Node*& removeInR(Node*& sub,const T& value) {

            if (sub == nullptr)
                return sub;
// keep finding the target node recusively
            if (value < sub->data_) {
                sub->left_ = removeInR(sub->left_, value);
            } else if (value > sub->data_) {
                sub->right_ = removeInR(sub->right_, value);
            }
        // found the target
            else {
                // if no child
                if (sub->left_ == nullptr && sub->right_ == nullptr ){
                    delete sub;
                    sub = nullptr;
                    return sub;
                }
                // if one child
                else if (sub->left_ == nullptr) {
                    Node * tem = sub;
                    sub = sub->right_;
                    delete tem;
                    return sub;
                } else if (sub->right_ == nullptr)
                { Node * tem = sub;
                    sub = sub->left_;
                    delete tem;
                    return sub;}
                
// (deletion on BST)if two child,
//find the smallest one in the right sub tree
//and use it to replace the deleted Node
else{Node *tem = new Node(Mini(sub->right_));
 tem->right_ = sub->right_;
tem->left_ = sub->left_;
delete sub;
sub = nullptr;
sub = tem;
sub->right_ = removeInR(sub->right_, sub->data_);} }
return sub;}
    
//--keep find the left-bottom value
T Mini(Node*& root) {
Node *tem = root;
T min =  tem->data_;
 while (tem->left_ != nullptr) {
min= tem->left_->data_;
tem = tem->left_;
}
return min;
	...
	int depth(const T& data){
        int sum = 0;
        //printPreOrder(root_);
        return depth(data, root_, sum);
	}
    // keep sum++ for each level;
    int depth(const T& data, Node* sub, int& sum){
        ...
        else if(  data > sub->data_ ){
            sum++;
            sum = depth(data, sub->right_, sum);
            return sum;
        }
        else {
            sum++;
            sum = depth(data, sub->left_, sum);
            return sum;
        }
